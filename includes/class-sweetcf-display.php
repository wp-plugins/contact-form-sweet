...
/**
 * Description of class-sweetcf-display
...
class SWEETCF_Display {
	...
	static $have_attach = '';
	static $printed_tooltip_filetypes, $swcf_use_window_onload;
	static $add_swcf_script, $add_placeholder_script, $add_date_js_array, $add_date_js;

	static function process_short_code($atts) { // Process shortcode and display the form, and decide whether to send the email or not
		self::$add_swcf_script = true;	// condition flag for add styles and scripts to page or post the form is on
		...
		// Verify form number
		self::$global_options = SWEETCF_Utils::get_global_options();

		self::$form_id_num = '1';
		if (isset($form) && is_numeric($form)) {
			self::$form_id_num = (int) $form;
		} else {
			echo __('Contact Form Shortcode Error: Invalid form number in shortcode.', 'sweetcontact');
			return;
		}
		...
		self::$form_options = SWEETCF_Utils::get_form_options(self::$form_id_num, false);	// Don't use defaults if it doesn't exist
		...
		if (self::$global_options['enable_php_sessions'] == 'true') { // this feature only works when PHP sessions are enabled
			...
			if (!empty($redirect) || !empty($hidden) || !empty($email_to)) {
				...
				// Display error message and return
				echo __('Contact Form Shortcode Error: Using shorcode attributes requires the PHP sessions setting to be enabled on the Advanced tab in form settings.', 'sweetcontact');
				return;
			}
		}
		...
		if (self::$form_options['external_style'] == 'true') { self::get_ext_css(); }
		// Has a preview been selected?
		$preview = ( isset($_POST['ctf_action']) && __('Preview Form', 'sweetcontact') == $_POST['ctf_action'] ) ? true : false;
		//if (is_admin() && $preview && self::$form_options['external_style'] == 'true') { self::external_style_head(); }
		...
		if (SWEETCF_Process::$form_processed && SWEETCF_Process::$form_id_num == self::$form_id_num && empty(SWEETCF_Process::$form_errors)) {
			...
			if (!empty(SWEETCF_Process::$form_errors) && SWEETCF_Process::$form_id_num == self::$form_id_num) {
				// The form was processed, but had errors
				if (!empty(SWEETCF_Process::$form_data)) {
					// If this is not true, there is an internal error...
					self::$form_content = array_merge(self::$form_content, SWEETCF_Process::$form_data);
					self::$form_errors = array_merge(self::$form_errors, SWEETCF_Process::$form_errors);
...
<div " . self::get_this_css('clear_style') . "></div>\n" . self::$form_options['welcome'];
	...
	static function get_ext_css() {
		...
		self::$ext_css = array(
			// Alignment DIVs
			'form_style' => 'swcf-div-form', // Form DIV, how wide is the form DIV
			'left_box_style' => 'swcf-div-left-box', // left box DIV, container for vcita
			'right_box_style' => 'swcf-div-right-box', // right box DIV, container for vcita
			'clear_style' => 'swcf-div-clear', // clear both
			'field_left_style' => 'swcf-div-field-left', // field left
			'field_prefollow_style' => 'swcf-div-field-prefollow', // field prefollow
			'field_follow_style' => 'swcf-div-field-follow', // field follow
			'title_style' => 'swcf-div-label', // Input labels alignment DIV
			'field_div_style' => 'swcf-div-field', // Input fields alignment DIV
			'captcha_div_style_sm' => 'swcf-div-captcha-sm', // Small CAPTCHA DIV
			'captcha_div_style_m' => 'swcf-div-captcha-m', // Large CAPTCHA DIV
			'captcha_image_style' => 'swcf-image-captcha', // CAPTCHA alignment
			'captcha_reload_image_style' => 'swcf-image-captcha-refresh', // CAPTCHA refresh image alignment
			'submit_div_style' => 'swcf-div-submit', // Submit DIV
			'border_style' => 'swcf-fieldset', // style of the fieldset box (if enabled)
			// Styles of labels, fields and text
			'required_style' => 'swcf-required-indicator', // required field indicator
			'required_text_style' => 'swcf-required-text', // required field text
			'hint_style' => 'swcf-hint-text', // small text hints like file types
			'error_style' => 'swcf-div-error', // Input validation messages
			'redirect_style' => 'swcf-div-redirecting', // Redirecting message
			'fieldset_style' => 'swcf-fieldset-field', // style of the fieldset box (for a field)
			'label_style' => 'swcf-label', // Field labels
			'option_label_style' => 'swcf-option-label', // Options labels
			'field_style' => 'swcf-input-text', // Input text fields
			'captcha_input_style' => 'swcf-input-captcha', // CAPTCHA input field
			'textarea_style' => 'swcf-input-textarea', // Input Textarea
			'select_style' => 'swcf-input-select', //  Input Select
			'checkbox_style' => 'swcf-input-checkbox', // Input checkbox
			'radio_style' => 'swcf-input-radio', // Input radio
			'button_style' => 'swcf-button-submit', // Submit button
			'reset_style' => 'swcf-button-reset', // Reset button
			'powered_by_style' => 'swcf-powered-by', // the "powered by" link
		);
	}
	...
	static function get_todays_honeypot_slug($fields) {
		// find a decoy field name that is not currently used on the form, change it each day of the week // TODO
		$decoy_fields = self::get_honeypot_slugs($fields);
		$max = count($decoy_fields);
		if ($max > 5) { $index = date('w'); } else if ($max > 0) { $index = 0; } else { return 'email456'; }
		return $decoy_fields[$index];
	}

	static function display_form($string) {
		// Build the code to display the form in $string and return it. The form code will be appended to $string and returned
		global $captcha_path_cf; // used by secureimage.php
		$captcha_path_cf = SWCF_CAPTCHA_PATH;
		...
		$string .= '
<div id="SWCFContact' . self::$form_id_num . '" ' . self::get_this_css('form_style') . ' class="sweetontact-wrapper">';

		$form_attributes = '';
		if (!empty(self::$form_options['form_attributes'])) { $form_attributes = self::$form_options['form_attributes'] . ' '; }
		$anchor = '';
		if (self::$form_options['anchor_enable'] == 'true') { $anchor = '#SWCFContact' . self::$form_id_num; }
		$string .= '<form class="sweetontact-form" ' . self::$have_attach . 'action="' . esc_url(self::$form_action_url) . $anchor . '" id="swcf_form' . self::$form_id_num . '" ' . $form_attributes . 'method="post">';
		if (self::$form_options['border_enable'] == 'true') {
			$string .= '<fieldset id="swcf_form_fieldset' . self::$form_id_num . '" ' . self::get_this_css('border_style') . ">\n";
		...
		$swcf_attach_dir = SWCF_ATTACH_DIR;
		if (self::$have_attach) {
			self::init_temp_dir(SWCF_ATTACH_DIR);
			if (self::$form_options['php_mailer_enable'] == 'php') {
				self::set_form_error("$swcf_attach_dir$frm_id", __('Attachments are only supported when the Send Email function is set to WordPress. You can find this setting on the contact form settings page.', 'sweetcontact'));
			}
			if (!is_dir(SWCF_ATTACH_DIR)) {
				self::set_form_error("$swcf_attach_dir$frm_id", __('The temporary folder for the attachment field does not exist.', 'sweetcontact'));
			} else if (!is_writable(SWCF_ATTACH_DIR)) {
				self::set_form_error("$swcf_attach_dir$frm_id", __('The temporary folder for the attachment field is not writable.', 'sweetcontact'));
			} else {
		...
		if (self::$contact_error) {
			...
			$string .= '      <div id="swcf_form_error' . self::$form_id_num . '" ' . self::get_this_css('error_style') . ">\n";
			$string .= (self::$form_options['error_correct'] != '') ? self::$form_options['error_correct'] : __('Please make corrections below and try again.', 'sweetcontact');
			$string .= "\n    </div>\n";

			// Print errors that appear at the top of the form
			$string .= self::echo_if_error("swcf_attach_dir$frm_id");
		}
		...
		if (empty(self::$contacts)) {
			$string .= '
		<div id="swcf_form_error_email' . self::$form_id_num . '" ' . self::get_this_css('error_style') . '>' . __('ERROR: Misconfigured email address in options.', 'sweetcontact')
				. "\n    </div>\n";
		}
		...
		if (is_admin()) {
			$hidden .= '<input type="hidden" name="ctf_action" value="' . __('Preview Form', 'sweetcontact') . '" />' . "\n";
		}
		$hidden .= '<input type="hidden" name="swcf_submitted" value="0" />' . "\n";
		$hidden .= '<input type="hidden" name="fs_postonce_' . self::$form_id_num . '" value="' . wp_hash(time()) . ',' . time() . '" />' . "\n";
		$hidden .= '<input type="hidden" name="sw_contact_action" value="send" />' . "\n";
		...
		if (self::$form_options['req_field_label_enable'] == 'true' && self::$form_options['req_field_indicator_enable'] == 'true') {
			$string .= "\n" . '<div id="swcf_required' . self::$form_id_num . '">' . "\n";
			...
			$string .= (self::$form_options['tooltip_required'] != '') ? self::$form_options['tooltip_required'] : __('indicates required field', 'sweetcontact');
			$string .= "</span>\n</div>\n\n";
		}
		// If there are multiple mail-to contacts, display a select form
		if (count(self::$contacts) > 1) {

			$string .= '<div id="swcf_div_clear_contact' . self::$form_id_num . '" ' . self::get_this_css('clear_style') . '>' . "\n"
				.'<div id="swcf_div_field_contact' . self::$form_id_num . '" ' . self::get_this_css('field_left_style') . '>'
				.'<div ' . self::get_this_css('title_style') . '>'
				.'<label ' . self::get_this_css('label_style') . ' for="swcf_mail_to' . self::$form_id_num . '">';
			$string .= (self::$form_options['title_dept'] != '') ? self::$form_options['title_dept'] : __('Select a contact:', 'sweetcontact');
			$string .= self::$req_field_ind . '</label>
    </div>
    <div ' . self::get_this_css('field_div_style') . '>' . self::echo_if_error('contact') . '
      <select ' . self::get_this_css('select_style') . ' id="swcf_mail_to' . self::$form_id_num . '" name="mailto_id" ' . self::$aria_required . '>';
			$string .= '       <option value="">';
			$string .= (self::$form_options['title_select'] != '') ? esc_html(self::$form_options['title_select']) : esc_html(__('Select', 'sweetcontact'));
			$string .= "</option>\n";
...
';
		...
		// A div class="swcf-clear" is used to group a field with any that follow it
		...
		foreach (self::$form_options['fields'] as $key => $field) {
			if ('true' != $field['disable']) { $field_follow[] = $field['follow']; }
		}

		$fld_cnt = 0;
		$fields_in_use = array();
		$string .= "\n" . '<div class="sweetontact-form-fields-wrapper">' . "\n";
		foreach (self::$form_options['fields'] as $key => $field) {
			//var_export($field);
			// fill in any missing field options defaults
			foreach ($field_opt_defaults as $dfkey => $dfval) {
				if (!isset($field[$dfkey]) || empty($field[$dfkey])) { $field[$dfkey] = $dfval; }
			}
			//echo '<hr>';var_export($field);

			if ('true' == $field['disable']) { continue; }
			
			$fields_in_use[$field['slug']] = 1;
			if ('true' == $field['follow'] && $open_div) {
				$string .= '  <div id="swcf_div_follow' . self::$form_id_num . '_' . $key . '" ' . self::get_this_css('field_follow_style') . '>';
			} else {
				if ($open_div) {
				...
				if ('fieldset' == $field['type'] || 'fieldset-close' == $field['type']) {
					...
					$open_div = true;
					$textarea_class = ( SWCF_MESSAGE_FIELD == $field['standard'] ) ? ' message-textarea-wrapper' : '';
					$string .= "\n" . '<div class="sweetontact-form-field-wrapper'.$textarea_class.' " id="swcf_div_clear' . self::$form_id_num . '_' . $key . '" ' . self::get_this_css('clear_style') . '>' . "\n" . '  ';
					$string .= '<div id="swcf_div_field' . self::$form_id_num . '_' . $key . '" ';
					...
					if (( isset($field_follow[$fld_cnt + 1]) && $field_follow[$fld_cnt + 1] == 'true' )
						|| ( SWCF_NAME_FIELD == $field['standard'] && 'name' != self::$form_options['name_format'] && 'true' == $field['inline'] ) 
					) {
			...
			if (!in_array($field['type'], array('fieldset', 'fieldset-close', 'hidden'))) {
				...
				if (('checkbox' != $field['type'] && $field['standard'] < 1) && ( 'false' == $field['hide_label'] )) {
					...
					$string .= "\n    <div " . ' id="swcf_label' . self::$form_id_num . '_' . $key . '" ' . self::get_this_css('title_style') . '>
      <label ';
					$string .= ( $field['label_css'] != '') ? self::convert_css($field['label_css']) : self::get_this_css('label_style');
					$string .= ' for="swcf_field' . self::$form_id_num . '_' . $key . '">' . esc_html($field['label']);
			...
			switch ($field['type']) {
				case 'fieldset' :
					if ($open_fieldset) { $string .= "</fieldset>\n"; }
					if ($field['notes'] != '') { $string .= "\n" . self::swcf_notes($field['notes']); }
					$string .= "\n<fieldset" . ' id="swcf_fieldset' . self::$form_id_num . '_' . $key . '" ';
					$string .= ($field['label_css'] != '') ? self::convert_css($field['label_css']) : self::get_this_css('fieldset_style');
					$string .= '>';
					if ('false' == $field['hide_label']) { $string .= '	  <legend>' . esc_html($field['label']) . "</legend>\n"; }
					$open_fieldset = true;
					break;
				...
				case 'password' :
					...
					$string .= ' type="password" id="swcf_field' . self::$form_id_num . '_' . $key . '" name="' . $field['slug'] . '" value=""'
//					No defaults for password!
						. '" ' . self::$aria_required . ' ';
					if ($field['max_len'] != '') { $string .= ' maxlength="' . $field['max_len'] . '"'; }
					if ($field['attributes'] != '') { $string .= ' ' . $field['attributes']; }
					$string .= " />\n    </div>\n";
					break;
			...
			if ('fieldset' <> $field['type'] && 'fieldset-close' <> $field['type'] && 'hidden' <> $field['type']) {
				if ($field['notes_after'] != '') {
					$string .= self::swcf_notes($field['notes_after']) . "\n";
				}
			}
			if ($open_div) {
				$string .= "  </div>\n"; // close field div
		...
		$string .= "\n" . '</div>' . "\n"; // end of sweetontact-form-fields-wrapper
		
		// action hook for form display after fields
		$string = apply_filters('sw_contact_display_after_fields', $string, self::$style, self::$form_errors, self::$form_id_num);

		if (count($date_fields) > 0) { self::setup_calendar($date_fields); } // Are there any date fields?
		...
		if (self::$form_options['design_type'] == '2') { // Vertical Design
			wp_enqueue_style('sweetcf-styles-vertical', SWCF_INCLUDES . '/sweetcf-styles-vertical.css', false, SWCF_BUILD);
		} else {
			wp_enqueue_style('sweetcf-styles-horizontal', SWCF_INCLUDES . '/sweetcf-styles-horizontal.css', false, SWCF_BUILD);
		}
...
<div style="display:none;">
     <label for="' . $honeypot_slug . self::$form_id_num . '"><small>' . __('Leave this field empty', 'sweetcontact') . '</small></label>
...
';
		...
		$string .= "\n<div " . 'id="swcf_submit_div' . self::$form_id_num . '" ' . self::get_this_css('submit_div_style') . '>
		<input type="submit" id="swcf_submit' . self::$form_id_num . '" ' . self::get_this_css('button_style') . ' value="';
		$string .= (self::$form_options['title_submit'] != '') ? esc_attr(self::$form_options['title_submit']) : esc_attr(__('Submit', 'sweetcontact'));
		$string .= '" ';
		if (!empty(self::$form_options['submit_attributes'])) { $string .= self::$form_options['submit_attributes'] . ' '; }
		if (self::$form_options['enable_areyousure'] == 'true') {
			$msg = (self::$form_options['title_areyousure'] != '') ? esc_html(addslashes(self::$form_options['title_areyousure'])) : esc_html(addslashes(__('Are you sure?', 'sweetcontact')));
			$string .= ' onclick="return confirm(\'' . $msg . '\')" ';
		}
		$string .= '/> ';

		if (self::$form_options['enable_reset'] == 'true') {
			$string .= '<input type="reset" id="swcf_reset' . self::$form_id_num . '" ' . self::get_this_css('reset_style') . ' value="';
			$string .= (self::$form_options['title_reset'] != '') ? esc_attr(self::$form_options['title_reset']) : esc_attr(__('Reset', 'sweetcontact'));
			$msg = addslashes(__('Do you really want to reset the form?', 'sweetcontact'));
			$string .= '" onclick="return confirm(\'' . $msg . '\')" />';
		}
...
';
	...
	static function display_name($key, $field) {
		...
		if (isset(self::$form_content[$field['slug']]) &&
			...
			$user_ID != '' &&
			$current_user->user_login != 'admin' && !current_user_can('manage_options') && self::$form_options['auto_fill_enable'] == 'true') {
		...
		if ($field['default'] != '') {
			if (self::$form_options['name_format'] == 'first_last') {
				if (!preg_match('/^(.*)(==)(.*)$/', $field['default'], $matches)) {	$field['default'] = 'First Name==Last Name'; } // default to proper format
				if (preg_match('/^(.*)(==)(.*)$/', $field['default'], $matches)) {
					$f_default = $matches[1];
					$l_default = $matches[3];
				}
			} else if (self::$form_options['name_format'] == 'first_middle_last') {
				if (!preg_match('/^(.*)(==)(.*)(==)(.*)$/', $field['default'], $matches)) { $field['default'] = 'First Name==Middle Name==Last Name'; } // default to proper format
				if (preg_match('/^(.*)(==)(.*)(==)(.*)$/', $field['default'], $matches)) {
					$f_default = $matches[1];
					$m_default = $matches[3];
					$l_default = $matches[5];
				...
				if (!preg_match('/^(.*)(==)(.*)(==)(.*)$/', $field['default'], $matches)) { $field['default'] = 'First Name==Middle Initial==Last Name'; } // default to proper format
				if (preg_match('/^(.*)(==)(.*)(==)(.*)$/', $field['default'], $matches)) {
					$f_default = $matches[1];
    ...
    <div ' . self::get_this_css('title_style') . ">\n";
		if ('false' == $field['hide_label']) {
			...
			$f_name_string .= ' for="swcf_f_name' . self::$form_id_num . '">';
			$f_name_string .= (self::$form_options['title_fname'] != '') ? self::$form_options['title_fname'] : __('First Name:', 'sweetcontact');
			if ('true' == $field['req']) { $f_name_string .= self::$req_field_ind; }
			$f_name_string .= "</label>\n";
		}
		$f_name_string .= '    </div>
    <div ' . self::get_this_css('field_div_style') . '>' . self::echo_if_error('f_name') . '
	  <input ';
		...
		$f_name_string .= ' type="text" id="swcf_f_name' . self::$form_id_num .
			'" name="f_name" value="' . esc_attr(self::$form_content['f_name']) . '" ' . self::$aria_required;
		if ($field['attributes'] != '') { $f_name_string .= ' ' . $field['attributes']; }
    ...
    <div ' . self::get_this_css('title_style') . ">\n";
		if ('false' == $field['hide_label']) {
			...
			$l_name_string .= ' for="swcf_l_name' . self::$form_id_num . '">';
			$l_name_string .= (self::$form_options['title_lname'] != '') ? self::$form_options['title_lname'] : __('Last Name:', 'sweetcontact');
			if ('true' == $field['req']) { $l_name_string .= self::$req_field_ind; }
			$l_name_string .= "</label>\n";
		}
		$l_name_string .= '    </div>
    <div ' . self::get_this_css('field_div_style') . '>' . self::echo_if_error('l_name') . '
	  <input ';
		...
		$l_name_string .= ' type="text" id="swcf_l_name' . self::$form_id_num . '" name="l_name" value="' . esc_attr(self::$form_content['l_name']) . '" ' . self::$aria_required;
		if ($field['attributes'] != '') { $l_name_string .= ' ' . $field['attributes']; }
...
';
    ...
    <div ' . self::get_this_css('title_style') . ">\n";
				if ('false' == $field['hide_label']) {
					$string .= '      <label ';
					$string .= ( $field['label_css'] != '') ? self::convert_css($field['label_css']) : self::get_this_css('label_style');
					$string .= ' for="swcf_name' . self::$form_id_num . '">';
					//$string .= (self::$form_options['title_name'] != '') ? self::$form_options['title_name'] : __('Name:', 'sweetcontact');
					$string .= __($field['label'], 'sweetcontact');
					if ('true' == $field['req']) { $string .= self::$req_field_ind; }
					$string .= "</label>\n";
				}
				$string .= '    </div>
    <div ' . self::get_this_css('field_div_style') . '>' . self::echo_if_error('full_name') . '
      <input ';
				$string .= ($field['input_css'] != '') ? self::convert_css($field['input_css']) : self::get_this_css('field_style');
				$string .= ' type="text" id="swcf_name' . self::$form_id_num . '" name="full_name" value="' . esc_attr(self::$form_content[$field['slug']]) . '" ' . self::$aria_required;
				if ($field['attributes'] != '')
...
';
			...
			case 'first_last':
				...
				if ('true' == $field['inline']) { $string .= '</div><div ' . self::get_this_css('field_follow_style') . '>'; }
    ...
    <div ' . self::get_this_css('title_style') . ">\n";
				if ('false' == $field['hide_label']) {
					$string .= '      <label ';
					$string .= ( $field['label_css'] != '') ? self::convert_css($field['label_css']) : self::get_this_css('label_style');
					$string .= ' for="swcf_mi_name' . self::$form_id_num . '">';
					$string .= (self::$form_options['title_miname'] != '') ? self::$form_options['title_miname'] : __('Middle Initial:', 'sweetcontact');
					$string .= "</label>\n";
				}
				$string .= '    </div>
    <div ' . self::get_this_css('field_div_style') . '>' . self::echo_if_error('mi_name') . '
       <input ';
				$string .= ($field['input_css'] != '') ? self::convert_css($field['input_css']) : self::get_this_css('field_style');
				$string .= ' type="text" id="swcf_mi_name' . self::$form_id_num . '" name="mi_name" value="' . esc_attr(self::$form_content['mi_name']) . '" ';
				if ($field['attributes'] != '') { $string .= ' ' . $field['attributes']; }
...
';
    ...
    <div ' . self::get_this_css('title_style') . ">\n";
				if ('false' == $field['hide_label']) {
					$string .= '      <label ';
					$string .= ( $field['label_css'] != '') ? self::convert_css($field['label_css']) : self::get_this_css('label_style');
					$string .= ' for="swcf_m_name' . self::$form_id_num . '">';
					$string .= (self::$form_options['title_mname'] != '') ? self::$form_options['title_mname'] : __('Middle Name:', 'sweetcontact');
					$string .= "</label>\n";
				}
				$string .= '    </div>
    <div ' . self::get_this_css('field_div_style') . '>' . self::echo_if_error('m_name') . '
      <input ';
				$string .= ($field['input_css'] != '') ? self::convert_css($field['input_css']) : self::get_this_css('field_style');
				$string .= ' type="text" id="swcf_m_name' . self::$form_id_num . '" name="m_name" value="' . esc_attr(self::$form_content['m_name']) . '" ' . self::$aria_required;
				if ($field['attributes'] != '') { $string .= ' ' . $field['attributes']; }
	...
	static function display_email($key, $field) {
		...
		$email_input_type = apply_filters('sw_contact_email_input_type', 'text', self::$form_id_num);
		...
		if ('' == self::$form_content[$field['slug']] && $user_ID != '' && $current_user->user_login != 'admin' &&
			...
			if ('true' == self::$form_options['double_email']) { self::$form_content['email2'] = $current_user->user_email; }
		}
		$email_default = '';
		$email2_default = '';
		if ('true' == self::$form_options['double_email']) {
			...
			// is there xx==xx
			if (!preg_match('/^(.*)(==)(.*)$/', $default, $matches)) { $default = 'Email==Re-enter Email'; }
		...
		if ('false' == $field['hide_label']) {
			$string .= '      <label ';
			$string .= ( $field['label_css'] != '') ? self::convert_css($field['label_css']) : self::get_this_css('label_style');
			$string .= ' for="swcf_email' . self::$form_id_num . '">';
			$string .= __($field['label'], 'sweetcontact'); //(self::$form_options['title_email'] != '') ? self::$form_options['title_email'] : __('Email:', 'sweetcontact');
			if ('true' == $field['req']) { $string .= self::$req_field_ind; }
			$string .= "</label>\n";
		}
		$string .= "    </div>\n";
		$string .= '    <div ' . self::get_this_css('field_div_style') . '>' . self::echo_if_error('email')
			. "\n      <input ";
		$string .= ($field['input_css'] != '') ? self::convert_css($field['input_css']) : self::get_this_css('field_style');
		$string .= ' type="' . $email_input_type . '" id="swcf_email' . self::$form_id_num . '" name="email" value="';
		...
		if ('true' == self::$form_options['double_email']) {
			$string .= "  </div>\n</div>\n\n" . '<div id="swcf_div_clear' . self::$form_id_num . '_' . $key . '_2" ' . self::get_this_css('clear_style') . '>' . "\n" . '  <div id="swcf_div_field' . self::$form_id_num . '_' . $key . '_2" ' . self::get_this_css('field_left_style') . '>';
			$string .= "\n    <div " . self::get_this_css('title_style') . ">\n";
			if ('false' == $field['hide_label']) {
				$string .= '      <label ';
				$string .= ( $field['label_css'] != '') ? self::convert_css($field['label_css']) : self::get_this_css('label_style');
				$string .= ' for="swcf_email' . self::$form_id_num . '_2">';
				$string .= (self::$form_options['title_email2'] != '') ? self::$form_options['title_email2'] : __('Re-enter Email:', 'sweetcontact');
				if ('true' == $field['req']) { $string .= self::$req_field_ind; }
				$string .= "</label>\n";
			}
			$string .= "    </div>\n    <div " . self::get_this_css('field_div_style') . '>' . self::echo_if_error('email2')
				. "\n      <input ";
			$string .= ($field['input_css'] != '') ? self::convert_css($field['input_css']) : self::get_this_css('field_style');
			$string .= ' type="' . $email_input_type . '" id="swcf_email' . self::$form_id_num . '_2" name="email2" value="' . esc_attr(self::$form_content['email2']) . '" ' . self::$aria_required;
			if ($field['attributes'] != '') { $string .= ' ' . $field['attributes']; }  // XXX same as email 1 though 
	...
	static function display_field_text($key, $field) { // display function for a text, email, or url field type
		$string = '';
		if (SWCF_NAME_FIELD == $field['standard']) { return(self::display_name($key, $field)); }
		if (SWCF_EMAIL_FIELD == $field['standard']) { return(self::display_email($key, $field)); }
		if (SWCF_SUBJECT_FIELD == $field['standard']) {
			...
			if ('false' == $field['hide_label']) {
				$string .= '      <label ';
				$string .= ( $field['label_css'] != '') ? self::convert_css($field['label_css']) : self::get_this_css('label_style');
				$string .= ' for="swcf_field' . self::$form_id_num . '_' . $key . '">';
				$string .= __($field['label'], 'sweetcontact'); //$string .= (self::$form_options['title_subj'] != '') ? self::$form_options['title_subj'] : __('Subject:', 'sweetcontact');
				if ('true' == $field['req']) { $string .= self::$req_field_ind; }
				$string .= "</label>\n";
			}
			$string .= "    </div>";
		...
		$type = $field['type'];
		if ('email' == $field['type'] || 'url' == $field['type']) { $type = 'text'; } // TODO
		$string .= "\n    <div " . self::get_this_css('field_div_style') . '>' . self::echo_if_error($field['slug']) . "\n" . '      <input ';
		$string .= ($field['input_css'] != '') ? self::convert_css($field['input_css']) : self::get_this_css('field_style');
		$string .= ' type="' . $type . '" id="swcf_field' . self::$form_id_num . '_' . $key . '" name="' . $field['slug'] . '" value="';
	...
	static function display_field_textarea($key, $field) {
		$string = '';
		if (SWCF_MESSAGE_FIELD == $field['standard']) { // Standard field labels can be changed in settings, so print that here
			$string .= "\n  <div " . self::get_this_css('title_style') . ">\n";
			if ('false' == $field['hide_label']) {
				$string .= '      <label ';
				$string .= ( $field['label_css'] != '') ? self::convert_css($field['label_css']) : self::get_this_css('label_style');
				$string .= ' for="swcf_field' . self::$form_id_num . '_' . $key . '">';
				$string .= __($field['label'], 'sweetcontact'); //$string .= (self::$form_options['title_mess'] != '') ? self::$form_options['title_mess'] : __('Message:', 'sweetcontact');
				if ('true' == $field['req']) { $string .= self::$req_field_ind; }
				$string .= "</label>\n";
			}
			...
			$string .= ' id="swcf_field' . self::$form_id_num . '_' . $key . '" name="message" cols="30" rows="10" ' . self::$aria_required;
			if ($field['attributes'] != '') { $string .= ' ' . $field['attributes']; }
			if ('true' == $field['placeholder'] && $field['default'] != '') {
				$string .= ' placeholder="' . esc_attr($field['default']) . '"';
				self::$placeholder = 1;
			...
			$string .= "\n    <div " . self::get_this_css('field_div_style') . '>' . self::echo_if_error($field['slug']) . "\n" . '<textarea ';
			$string .= ($field['input_css'] != '') ? self::convert_css($field['input_css']) : self::get_this_css('textarea_style');
			$string .= ' id="swcf_field' . self::$form_id_num . '_' . $key . '" name="' . $field['slug'] . '" cols="30" rows="10" ' . self::$aria_required;
			if ($field['attributes'] != '') { $string .= ' ' . $field['attributes']; }
			if ('true' == $field['placeholder'] && $field['default'] != '') {
				$string .= ' placeholder="' . esc_attr($field['default']) . '"';
			}
	...
	static function display_field_select($key, $field) {
		...
		if ('' == $opts_array[0]) {
			...
			self::set_form_error("swcf_select$frm_id", __('Error: No options were entered for a select field in settings.', 'sweetcontact'));
			$string .= self::echo_if_error("swcf_select$frm_id");
		}
		...
		if (SWCF_SUBJECT_FIELD == $field['standard']) {
			...
			if ('false' == $field['hide_label']) {
				...
				$string .= ( $field['label_css'] != '') ? self::convert_css($field['label_css']) : self::get_this_css('label_style');
				$string .= ' for="swcf_field' . self::$form_id_num . '_' . $key . '">';
				$string .= (self::$form_options['title_subj'] != '') ? self::$form_options['title_subj'] : __('Subject:', 'sweetcontact');
				if ('true' == $field['req']) { $string .= self::$req_field_ind; }
				$string .= "</label>\n";
			}
			$string .= "    </div>";
		...
		$string .= ' id="swcf_field' . self::$form_id_num . '_' . $key . '" name="' . $field['slug'] . '[]"';
		if ($field['attributes'] != '') { $string .= ' ' . $field['attributes']; }
		$string .= $mult . ">\n";
		...
		foreach ($opts_array as $opt) {
			...
			if (is_array(self::$form_content[$field['slug']])) {
				if (count(self::$form_content[$field['slug']]) > 0) {
					if (in_array($opts_cnt, self::$form_content[$field['slug']])) { $selected = ' selected="selected"'; }
				}
			}
	...
	static function display_field_checkbox($key, $field) {
		...
		if ('' == $opts_array[0]) {
			if ('checkbox' == $field['type']) {
				...
				self::$form_errors['swcf_checkbox'] = __('Error: No options were entered for a checkbox-multiple field in settings.', 'sweetcontact');
				$string .= self::echo_if_error('swcf_checkbox');
			}
		}
		if ('checkbox' == $field['type']) {
			// Single checkbox
			...
			$string .= ' id="swcf_field' . self::$form_id_num . '_'
			...
			$string .= ' for="swcf_field' . self::$form_id_num . '_' . $key . '">' . $field['label'];
			$string .= ( 'true' == $field['req'] ) ? self::$req_field_ind : '';
			$string .= "</label></span>\n    </div>\n";
		} else {
			...
			foreach ($opts_array as $opt) {
				$opt = trim($opt);
				if ('false' == $field['inline'] && $opt_cnt > 1) { $string .= "<br />\n"; }
				else if ($opt_cnt > 1) { $string .= "\n"; }
				$string .= '      <span><input type="' . $ftype . '" ';
				if ($ftype == 'radio') { $string .= ($field['input_css'] != '') ? self::convert_css($field['input_css']) : self::get_this_css('radio_style'); }
				else { $string .= ($field['input_css'] != '') ? self::convert_css($field['input_css']) : self::get_this_css('checkbox_style'); }
				$string .= ' id="swcf_field' . self::$form_id_num . '_' . $key . '_' . $opt_cnt . '" name="' . $field['slug'];
				if ('checkbox' == $ftype) {
				...
				if ('checkbox-multiple' == $field['type']) {
					if (count(self::$form_content[$field['slug']]) > 0 && in_array($opt_cnt, self::$form_content[$field['slug']])) {
						$string .= ' checked="checked"';
					}
				...
				$string .= ' /> <label ';
				$string .= ( $field['label_css'] != '') ? self::convert_css($field['label_css']) : self::get_this_css('option_label_style');
				$string .= ' for="swcf_field' . self::$form_id_num . '_' . $key . '_'
	...
	static function display_field_date($key, $field) {
		...
		$string .= "\n    <div " . self::get_this_css('field_div_style') . '>' . self::echo_if_error($field['slug']) .
			"\n      <input ";
		$string .= ($field['input_css'] != '') ? self::convert_css($field['input_css']) : self::get_this_css('field_style');
		$string .= ' type="text" id="swcf_field' . self::$form_id_num . '_' . $key . '" name="' . $field['slug'] . '" value="';
	...
	static function display_field_attachment($key, $field) {
		...
		if (self::$form_options['php_mailer_enable'] != 'php') {
			$string .= "\n    <div " . self::get_this_css('field_div_style') . '>' . self::echo_if_error($field['slug'])
				. "\n      <input " . self::get_this_css('field_style') . ' type="file" id="swcf_field'
			...
			if (!self::$printed_tooltip_filetypes) {
				$file_type_message = "<br />\n      <span " . 'id="swcf_hint_file_types' . self::$form_id_num . '_' . $key . '" ' . self::get_this_css('hint_style') . '>';
				$file_type_message .= sprintf((self::$form_options['tooltip_filetypes'] != '') ? self::$form_options['tooltip_filetypes'] : __('Acceptable file types: %s.', 'sweetcontact'), self::$form_options['attach_types']);
				$file_type_message .= '<br />';
				$file_type_message .= sprintf((self::$form_options['tooltip_filesize'] != '') ? self::$form_options['tooltip_filesize'] : __('Maximum file size: %s.', 'sweetcontact'), self::$form_options['attach_size']) . "</span>\n";
				//filter hook for file attachment acceptable types message
				$file_type_message = apply_filters('sw_contact_file_type_message', $file_type_message, self::$form_options, self::$form_id_num);
	...
	static function setup_calendar($date_fields) {
		...
		if (self::$add_date_js == '') { // only add for 1st form with date fields
			self::$add_date_js = '
<!-- sweetContact Form plugin - begin date field js - form ' . self::$form_id_num . ' -->
<script type="text/javascript">
  var ctf_daylist = new Array( \'' . __('Su', 'sweetcontact') . '\',\'' . __('Mo', 'sweetcontact') . '\',\'' . __('Tu', 'sweetcontact') . '\',\'' . __('We', 'sweetcontact') . '\',\'' . __('Th', 'sweetcontact') . '\',\'' . __('Fr', 'sweetcontact') . '\',\'' . __('Sa', 'sweetcontact') . '\',\'' . __('Su', 'sweetcontact') . '\',\'' . __('Mo', 'sweetcontact') . '\',\'' . __('Tu', 'sweetcontact') . '\',\'' . __('We', 'sweetcontact') . '\',\'' . __('Th', 'sweetcontact') . '\',\'' . __('Fr', 'sweetcontact') . '\',\'' . __('Sa', 'sweetcontact') . '\' );
  var ctf_months_sh = new Array( \'' . __('Jan', 'sweetcontact') . '\',\'' . __('Feb', 'sweetcontact') . '\',\'' . __('Mar', 'sweetcontact') . '\',\'' . __('Apr', 'sweetcontact') . '\',\'' . __('May', 'sweetcontact') . '\',\'' . __('Jun', 'sweetcontact') . '\',\'' . __('Jul', 'sweetcontact') . '\',\'' . __('Aug', 'sweetcontact') . '\',\'' . __('Sep', 'sweetcontact') . '\',\'' . __('Oct', 'sweetcontact') . '\',\'' . __('Nov', 'sweetcontact') . '\',\'' . __('Dec', 'sweetcontact') . '\' );
  var ctf_monthup_title = \'' . __('Go to the next month', 'sweetcontact') . '\';
  var ctf_monthdn_title = \'' . __('Go to the previous month', 'sweetcontact') . '\';
  var ctf_clearbtn_caption = \'' . __('Clear', 'sweetcontact') . '\';
  var ctf_clearbtn_title = \'' . __('Clears any dates selected on the calendar', 'sweetcontact') . '\';
  var ctf_maxrange_caption = \'' . __('This is the maximum range', 'sweetcontact') . '\';
  ...
  var ctf_date_format = \'';
			...
			self::$add_date_js .= '\';
';
			self::$swcf_use_window_onload = true;
			//filter hook to suppress window.onload = function(){} for sites where it can only happen once per page.
			self::$swcf_use_window_onload = apply_filters('sw_contact_use_window_onload', self::$swcf_use_window_onload, self::$form_id_num);
		}
	}
	...
	static function display_field_time($key, $field) {
		...
		$string .= "      </select>:\n      <select ";
		$string .= ($field['input_css'] != '') ? self::convert_css($field['input_css']) : self::get_this_css('select_style');
		$string .= ' id="swcf_field' . self::$form_id_num . '_' . $key . 'm" name="' . $field['slug'] . '[m]">' . "\n";
		$selected = '';
		// minutes
		$string .= '        <option value=""></option>' . "\n";
		...
		if (self::$form_options['time_format'] == '12') {
			$string .= "\n      <select ";
			$string .= ($field['input_css'] != '') ? self::convert_css($field['input_css']) : self::get_this_css('select_style');
			$string .= ' id="swcf_field' . self::$form_id_num . '_' . $key . 'ap" name="' . $field['slug'] . '[ap]">' . "\n";
			$selected = '';
			// am/pm
			$string .= '        <option value=""></option>' . "\n";
			foreach (array(esc_html(__('AM', 'sweetcontact')), esc_html(__('PM', 'sweetcontact'))) as $k) {
	...
	static function get_var($form_id_num, $name) {
		...
		$value = (isset($_GET["$form_id_num$name"])) ? SWEETCF_Utils::clean_input($_GET["$form_id_num$name"]) : '';
		return $value;
	}
	...
	static function get_query_parms() {
		...
		foreach ($swcf_special_slugs as $fld_name) {
		...
		foreach (self::$form_options['fields'] as $key => $field) {
			...
			switch ($field['type']) {
				...
				case 'date' :
					...
					if ('' != $new_date & SWEETCF_Process::validate_date($new_date, self::$form_id_num))
						self::$form_content[$fld_name] = $new_date;
					// XXX Consider adding an error or warning message if date field query parm is invalid
					if (empty(self::$form_content[$fld_name]) && '' != $field['default'] && SWEETCF_Process::validate_date($field['default'], self::$form_id_num)) {
	...
	static function get_hidden_fields() {
		// Gets hidden fields from shortcode. This is called only if PHP Sessions are enabled. Returns an array of hidden fields
		$frm_id = self::$form_id_num;
		$hidden_fields = array();
	...
	static function echo_if_error($this_error) { // Display contact form errors
		// Settings errors begin with swcf . $form_num (always shown)
	...
	static function init_temp_dir($dir) { 	// needed for making temp directories for attachments and captcha session files
		$dir = trailingslashit($dir);
		wp_mkdir_p($dir);	// make the temp directory
		//@chmod( $dir, 0733 );
	...
	static function is_captcha_enabled($frm_num) { // Check if captcha is enabled for this user and this form
		if (!self::$form_options) {
			self::$form_options = SWEETCF_Utils::get_form_options($frm_num, $use_defaults = true);
		}
		$captcha_enabled = (self::$form_options['captcha_enable'] == 1);
		...
		if (is_user_logged_in() && self::$form_options['captcha_perm'] == 'true') {
			...
			if (current_user_can(self::$form_options['captcha_perm_level'])) { $captcha_enabled = false; }
		}
		*/
	...
	static function display_captcha() {	// this function adds the captcha to the contact form
		...
		if ( SWCF_CAPTCHA_OK ) {
			if ( is_admin() ) {
				...
				echo "<span style='color: green'>".__('Message will be sent to', 'sweetcontact').": <span style='font-weight:bold;'>". $email_to . "</span></span>";
				$html .= "\n<div " . self::get_this_css('title_style') . ">\n</div>\n" . '<div id="swcf_captcha_image_div' . self::$form_id_num . '" ';
	...
	static function display_thank_you() {
		...
		$ctf_thank_you .= '<div id="swcf_redirect' . self::$form_id_num . '" ' . SWEETCF_Display::get_this_css('redirect_style') . '>';
		$text_message_sent = (self::$form_options['text_message_sent'] != '') ? self::$form_options['text_message_sent'] : __('Your message has been sent, thank you.', 'sweetcontact'); // can have HTML
		$ctf_thank_you .= $text_message_sent;
		if (SWEETCF_Process::$redirect_enable == 'true') {
			$ctf_thank_you .= '
		<br />
		<img id="swcf_redirect_image' . self::$form_id_num . '" src="' . SWCF_INCLUDES . '/loading.gif' . '" alt="' . esc_attr(__('Redirecting', 'sweetcontact')) . '" />' .
			...
			if (self::$form_options['print_form_enable'] == 'true') {
				...
				$ctf_thank_you .= '
<br />
<input type="button" id="swcf_print_button' . self::$form_id_num . '" value="';
				$ctf_thank_you .= (self::$form_options['text_print_button'] != '') ? self::$form_options['text_print_button'] : __('View / Print your message', 'sweetcontact');
				$ctf_thank_you .= '" onclick="fscfPrintContent(\'swcf_print_div\'); return false;" />
<div id="swcf_print_div" style="display:none">
<h2>' . $text_message_sent . '</h2>

<p>';

				$msg_print = esc_html(SWEETCF_Process::$email_msg_print);
...
';
	...
	static function set_form_error($fld, $msg) {
		...
		// This is called from SWEETCF_Process class functions
		// The key is 'field' plus the field index number, or a special name such as capctha, f_name, swcf_select, etc.
		self::$form_errors[$fld] = $msg;
		self::$contact_error = 1; // Set the error flag
	}

	static function check_form_errors() { // Returns 1 if there are form errors, and 0 if not
		return(self::$contact_error);
	}

	static function swcf_notes($notes) {
		$html = "\n    <div " . self::get_this_css('clear_style') . "></div>\n$notes\n";
		// filter hook for html_before_after
		return apply_filters('sw_contact_html_before_after', $html, self::$form_id_num);
	}

	static function get_form_action_url() {
		// returns the URL for the WP page the form was on
		if (function_exists('qtrans_convertURL')) { // compatible with qtranslate plugin. In case of multi-lingual pages, the /de/ /en/ language url is used.
		...
		if (is_ssl()) { $form_action_url = preg_replace('|http://|', 'https://', $form_action_url); }
		//filter hook for form action URL
		return apply_filters('sweetcontact_form_action_url', $form_action_url, self::$form_id_num);
	}
